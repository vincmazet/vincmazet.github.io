/*
    ...
    Version du 29/09/2016

    Copyright Vincent Mazet (vincent.mazet@unistra.fr) et co-auteurs, 2016.
...
function Graph(div, x, y, w, h)
{
    ...
    ctx.px2pt = function(px)
    {
        ...
        return [      px[0] * window.devicePixelRatio /  cnv.width  * (ctx.xlim[1] - ctx.xlim[0]) + ctx.xlim[0] ,
                 (1 - px[1] * window.devicePixelRatio / cnv.height) * (ctx.ylim[1] - ctx.ylim[0]) + ctx.ylim[0] ];
    };
    ...
    ctx.mouseMove = function(f)
    {
        cnv.addEventListener('mousemove', function(evt)
        {
          ...
          var p = ctx.px2pt([x/window.devicePixelRatio, y/window.devicePixelRatio]);
          x = p[0];
          y = p[1];
          f(x, y);
        });
    ...
    ctx.mouseDrag = function(f)
    {
        cnv.addEventListener('mousemove', function(evt)
        {
            ...
            var rect = cnv.getBoundingClientRect();
            var x = Math.round(evt.clientX - rect.left);// * window.devicePixelRatio;
            var y = Math.round(evt.clientY - rect.top);// * window.devicePixelRatio;
            var p = ctx.px2pt([x/window.devicePixelRatio, y/window.devicePixelRatio]);
            x = p[0];
            y = p[1];
            //console10.innerHTML = "MOUSEMOVE: x = " + x + ",  y = " + y;
            f(x, y);
        });
        cnv.addEventListener('touchmove', function(evt)
        {
            var rect = cnv.getBoundingClientRect();
            var x = Math.round(evt.targetTouches[0].pageX - rect.left);//;// * window.devicePixelRatio;
            var y = Math.round(evt.targetTouches[0].pageY - rect.top + document.body.getBoundingClientRect().top);// * window.devicePixelRatio;
            var p = ctx.px2pt([x/window.devicePixelRatio, y/window.devicePixelRatio]);
            x = p[0];
            y = p[1];
            f(x, y);
            //console10.innerHTML = "TOUCHMOVE: x = " + x + ",  y = " + y;
            evt.preventDefault();
        });
    };
