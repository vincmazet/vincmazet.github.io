...
{   
    //var divMod = document.getElementById(no);
    ...
    //console.log(gphMod);
    gphMod.clear();
    gphMod.strokeStyle = '#d0d0d0';
    gphMod.grid();
    gphMod.strokeStyle ='#0020e0';
    gphMod.plot(x,y);
}
...
function bittoggle(i) { return function()
{
    ...
    var divs = anim.getElementsByTagName('div');
    for(var j = 0; j < divs.length; j++)
    {
        //console.log('Redessine ' + divs[j].getAttribute('id'));
        drawMod(divs[j]);
    }
};}
...
{
    //console.log(divMod.getAttribute('id'));
    drawMod(divMod);
};}

// Supprime une modulation
// TODO : travailler sur les divs plutôt que leur Id ?
function supprMod(no) { return function()
{
    ...
    // Nouvel hauteur du div anim
    var animHeight = 2*bw + (M-1)*gh;
...
function addMod()
{
    ...
    divMod.id = idcounter;
    anim.appendChild(divMod);
    
    // Nom
    var selMod = new Select(divMod, 0, 0, lw-cs, mods);
    var pos = (gh-selMod.offsetHeight) / 2
    selMod.style.top = pos.toString() + 'px';
    selMod.addEventListener('change', changeMod(divMod), false);
    
    // Graphe
    gphMod = new Graph(divMod, lw, 0, B*bw, gh-2*cs);
    gphMod.xlim = [0, B];
    gphMod.ylim = [-1, 1];
    gphMod.strokeStyle = '#d0d0d0';
    gphMod.grid();
    ...
    btnMod = new Button(divMod, '&#10005;', lw+B*bw+2*cs, (gh-cw)/2, cw, cw, 'Supprimer');
    btnMod.addEventListener('click', supprMod(idcounter), false);
...
{
    // Div Animation
    anim = inidiv('modulations-numeriques', lw + (B+1)*bw, 2*bw + gh);
    
    // Boutons de la séquence binaire
    for (i = 0; i < B; i++) {
        btnBit[i] = new Button(anim, seq[i], lw+i*bw+cs, cs, cw, cw, '');
        btnBit[i].addEventListener('click', bittoggle(i), false);
    }
    
    // Bouton d'ajout de modulation
    btnAdd = new Button(anim, 'Ajouter une modulation', lw, bw+M*gh+cs, 6*bw, cw, 'Ajouter une modulation');
    btnAdd.addEventListener('click', addMod, false);
